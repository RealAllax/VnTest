...
namespace VNEngine
{
    public class Actor : MonoBehaviour
    {
        ...
        public List<SkeletonGraphic> skeleton;
        [HideInInspector]
        ...
        public void Reset()
        {
            ...
            foreach(SkeletonGraphic Skeleton in skeleton)
            {
        ...
        IEnumerator Fade_In_Coroutine(float over_time)
        {
            ...
            Color tmp_skeleton_color = skeleton[0].color;
            tmp_color.a = 0;
            tmp_skeleton_color.a = 0;
            float value = 0;

            while (value < over_time)
            {
                value += Time.deltaTime;
                tmp_skeleton_color.a = Mathf.Lerp(0, 1, value / over_time);
                tmp_color.a = Mathf.Lerp(0, 1, value / over_time);
                img.color = tmp_color;
                foreach (SkeletonGraphic Skeleton in skeleton)
                    Skeleton.color = tmp_skeleton_color;
                yield return 0;
            }
            yield break;
        }
        ...
        IEnumerator Fade_Out_Coroutine(float over_time)
        {
            ...
            Color tmp_skeleton_color = skeleton[0].color;
            tmp_color.a = 1;
            tmp_skeleton_color.a = 1;
            float value = 0;

            while (value < over_time)
            {
                value += Time.deltaTime;
                tmp_skeleton_color.a = Mathf.Lerp(1, 0, value / over_time);
        ...
        {
            float value = 0;
            while (this.GetComponent<Image>().color != Color.gray || skeleton[0].color != Color.gray)
            {
                value += brigthness_change;
                foreach (SkeletonGraphic Skeleton in skeleton)
                    Skeleton.color = Color.Lerp(Color.white, Color.black, value);
        ...
        public void Lighten()
        {
            if (this.GetComponent<Image>().color != Color.white || skeleton[0].color != Color.white)
            {
                StopAllCoroutines();
        ...
        {
            float value = 0;
            while (this.GetComponent<Image>().color != Color.white || skeleton[0].color != Color.white)
            {
                value += brigthness_change;
                foreach (SkeletonGraphic Skeleton in skeleton)
                    Skeleton.color = Color.Lerp(Color.gray, Color.white, value);
